    ...
    describe('RSA', () => {
        ...
        for (const algorithm of algorithms) {
            it(`supports ${algorithm}`, () => {
                ...
                const pairResult = Mock.pair();
                const signature = Jwt.token.signature.generate(value, algorithm, pairResult.private);
                expect(Jwt.token.signature.verify({ header, payload, signature }, algorithm, pairResult.public)).to.be.true();
            });
        }
    });
    ...
    describe('EC', () => {
        ...
        for (const algorithm of algorithms) {
            it(`supports ${algorithm}`, () => {
                ...
                const pairResult = Mock.pair('ec', algorithm.slice(2));
                const signature = Jwt.token.signature.generate(value, algorithm, pairResult.private);
                expect(Jwt.token.signature.verify({ header, payload, signature }, algorithm, pairResult.public)).to.be.true();
            });
        }
    ...
    describe('EdDSA', () => {
        ...
        for (const crv of curves) {
            it(`supports ${alg} ${crv}`, () => {
                ...
                const pairResult = Mock.pair(alg, crv);
                const signature = Jwt.token.signature.generate(value, alg, pairResult.private);
                expect(Jwt.token.signature.verify({ header, payload, signature }, alg, pairResult.public)).to.be.true();
            });
        }
    });
