...
describe('Token', () => {
    ...
    it('creates and verifies a token (EdDSA,ed25519)', () => {
        ...
        const pairResult = Mock.pair('EdDSA', 'ed25519');
        const token = Jwt.token.generate({ test: 'ok' }, { key: pairResult.private, algorithm: 'EdDSA' });

        const artifacts = Jwt.token.decode(token);
        Jwt.token.verify(artifacts, pairResult.public);
    ...
    it('creates and verifies a token (EdDSA,ed448)', () => {
        ...
        const pairResult = Mock.pair('EdDSA', 'ed448');
        const token = Jwt.token.generate({ test: 'ok' }, { key: pairResult.private, algorithm: 'EdDSA' });
