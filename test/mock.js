...
export const jwks = async function (options = {}) {
    ...
    if (options.public !== false) {
        ...
        cert.publicKey = Forge.pki.publicKeyFromPem(pairResult.public);
        ...
        cert.sign(Forge.pki.privateKeyFromPem(pairResult.private));
    ...
    if (options.rsa !== false) {
        const rsa = new Rsa();
        rsa.importKey(pairResult.private);
...
export const pair = function (type = 'rsa', bits = 2048) {

    // RSA

    if (type === 'rsa') {
        const pairResult = Forge.pki.rsa.generateKeyPair(bits);
        const keys = {
            private: Forge.pki.privateKeyToPem(pairResult.privateKey),
            public: Forge.pki.publicKeyToPem(pairResult.publicKey)
        };
