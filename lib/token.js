...
export const generate = function (payload, secret, options = {}) {
...
export const decode = function (token, options = {}) {
    ...
    if (parts.length === 3) {
        ...
        const headlessVal = headless(options);
        artifacts.token = `${headlessVal.raw}.${token}`;
        artifacts.raw = { header: headlessVal.raw, payload: parts[0], signature: parts[1] };
        artifacts.decoded.header = headlessVal.decoded;
    }
    else {
...
export const verify = function (artifacts, secret, options = {}) {

    verifySignature(artifacts, secret);
    verifyPayload(artifacts, options);
};


export const verifySignature = function ({ decoded, raw }, secret) {
...
export const verifyPayload = function ({ decoded }, options = {}) {
...
export const verifyTime = function ({ decoded }, options = {}, _nowSec = null) {
...
export const headless = function (options) {

    const headlessVal = options.headless;
    if (!headlessVal) {
        return null;
    }

    if (typeof headlessVal === 'object') {
        if (headlessVal[internals.headless]) {
            return headlessVal;
        }

        return {
            [internals.headless]: true,
            raw: Buffer.from(JSON.stringify(headlessVal)).toString('base64'),
            decoded: headlessVal
        };
    }
    ...
    return {
        [internals.headless]: true,
        raw: headlessVal,
        decoded: internals.b64parse(headlessVal)
    };
};
